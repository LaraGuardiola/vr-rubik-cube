...
- VR menu: a floating 3D panel (Scramble / Reset / Undo, canvas-texture labels) toggled with the **left controller's trigger on empty space**; beam + trigger on a button activates it.
...
- **Whole-cube grab in VR** = middle-finger pinch (hands) **within reach of the cube**, or the **grip** button (controllers) **while the laser is on the cube** (or the tip is within 0.25 m) — no accidental grabs from across the room. Controllers then need an explicit **pull gesture**: while holding grip on the cube, yank the controller back (away from the cube, ≥ 0.06 m) to trigger the gravity pull; releasing before the pull does nothing. The cube flies to a hold point ~0.1 m in front of the controller tip (Half-Life: Alyx style), locks on, follows the hand rigidly, then floats where released. (Hands: a middle pinch grabs directly — the hand is already at the cube.)
- **Hitbox feedback.** When a controller laser is on the cube, the targeted cubie gets a thick blue edge outline; when the controller tip is within **0.25 m** of the cube centre the cube's **edges** glow blue. Outlines are built from thin boxes (`src/outline.ts`) because WebGL ignores `linewidth > 1`; the cube keeps its sticker colours visible.
- **VR menu** (`src/menu.ts`) — a floating 3D panel with Scramble / Reset / Undo buttons (canvas-texture labels; regular DOM can't show inside an immersive session). The **left controller's trigger on empty space** toggles it; hovering a button outlines it blue; trigger on a button activates it; left trigger on empty space again closes it. It's anchored in front-and-to-the-right of the user so its beam isn't blocked by the cube.
