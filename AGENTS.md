...
- **Cubie size 0.1 m** (whole cube ~0.3 m) — reads as "a cube you hold" in VR and still allows pinching individual cubies. (Earlier 0.6 m at the old 2.6 m spawn read as gigantic.)
...
- **Hitbox feedback.** When a controller laser is on the cube, the targeted cubie glows blue; when the controller tip is within **0.35 m** of the cube centre the whole cube glows softly. Cubies use per-cubie material clones so individual highlights don't tint the rest of the cube.
...
1. **Verify VR on hardware** — pinch thresholds (3.5/5.5 cm), the 0.12 rad axis-ambiguity heuristic, the 0.09 m grab radius, and hand-rig scale all need a real pass on a headset.
