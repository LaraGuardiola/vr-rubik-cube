...
- **Hitbox feedback, distance-based.** Beyond 1 m the cube's **edge outline** glows with a neon pulse — but only while the laser line actually passes near the cube (`aimingAtCube`: perpendicular ray-to-centre distance < 0.45 m), so it never lingers when you're not pointing at it. Within 1 m, aiming at the cube shows the laser-targeted cubie outline AND the whole-cube glow. Outlines are thin box frames (`src/outline.ts`, ~3–4 mm bars) because WebGL ignores `linewidth > 1`; sticker colours stay readable.
- **Rotating-face outline.** While a layer is turning (drag or snap animation) the cube draws a frame around that slice/face (`showSliceOutline`), centred in the slice plane like the middle layer, and it **rotates with the slice** so you always see the active axis. While a turn is active only the face + the laser-targeted box stay highlighted (no full-cube glow).
- **Thumbstick turns.** While holding a piece with the trigger, the controller's thumbstick drives the layer: up/down → the vertical (Y) axis, left/right → the horizontal (X) axis, deflection = turn angle (snaps to 90° on release). The thumbstick is read from the input source's gamepad (found by handedness if the `connected` event is missed). The motion-based hand turn still works alongside.
- **Two-hand resize.** Hold the cube with one hand/controller (grip or middle pinch), then grab it with the other (within 0.7 m) and stretch your hands apart to enlarge or bring them together to shrink (0.4×–3×). The cube grows **away from the holding hand** (the held side stays stationary) and the size persists after release. Hitbox radii (proximity, aim cone, grab radius) scale with the cube's size.
