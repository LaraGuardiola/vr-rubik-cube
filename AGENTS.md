...
- **Whole-cube grab in VR** = middle-finger pinch (hands) **within reach of the cube**, or the **grip** button (controllers) **while the laser is on the cube** (or the tip is within 0.25 m) — no accidental grabs from across the room. If the grab point is far away the cube is **gravity-pulled** to the hand (Half-Life: Alyx style): it flies in with eased position + orientation, then locks on and follows the hand rigidly, then floats where released.
- **Hitbox feedback.** When a controller laser is on the cube, the targeted cubie glows blue; when the controller tip is within **0.25 m** of the cube centre the cube's **edges** glow blue (a `LineSegments` outline, not the whole cube). Cubies use per-cubie material clones so individual highlights don't tint the rest of the cube.
