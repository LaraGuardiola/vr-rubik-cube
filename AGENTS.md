...
2. **WebXR — hand-tracked immersive** — prefers **AR passthrough** (`immersive-ar`, cube floats in the player's real room); falls back to **immersive VR** (`immersive-vr`) with a fully procedural nebula/starfield skybox. Two input styles, both supported simultaneously:
   - **Hand tracking** (no controllers): pinch with thumb+index to grab a layer and twist to turn it; pinch in empty space near the cube to grab and move/rotate the whole cube.
   - **Quest controllers**: aim the laser at a face and pull the **trigger** to turn that slice; hold the **grip** button to grab and move the whole cube. (Or trigger in empty space near the cube.)
   When the player lets go, the cube stays floating exactly where it was released — no gravity, no drift.
...
| `src/controllers.ts` | `ControllerSource`: drives the same interactions with Quest controllers. Trigger (`select`) raycasts the laser to pick the slice under the beam and turns it by controller orbit; grip (`squeeze`) grabs the whole cube. Includes a small procedural controller body + aim beam. |
| `src/xrControls.ts` | `XRControls`: VR interaction state machine over any `PinchSource`. On grab start it decides **layer turn** (hand: pinched cubies share a slice; controller: laser hit) vs **whole-cube grab** (empty space, laser miss, or grip button). Drives `beginLiveTurn/setLiveAngle/endLiveTurn` for slices and rigidly attaches the cube to the hand/controller for whole-cube grabs. One active interaction at a time. |
...
- **Cubie size 0.16 m** (whole cube ~0.48 m) — small enough to read as "a cube you hold" in VR, still easy to pinch individual cubies. (Earlier 0.2 m read as huge at the old 2.6 m spawn.)
...
- **Whole-cube grab in VR** = pinch in empty space within **0.55 m** of the cube centre, grab cubies that don't line up on any single slice, aim a controller laser past the cube, or hold the **grip** button. The cube is then rigidly attached to the wrist/controller pose (position + orientation offset), so it follows the hand and stays where it is released. This doubles as the VR "inspect/rotate the cube" gesture.
